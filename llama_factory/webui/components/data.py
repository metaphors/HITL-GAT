...
def can_preview(dataset: list) -> "gr.Button":
    try:
        with open(os.path.join(DEFAULT_DATA_DIR, DATA_CONFIG), "r", encoding="utf-8") as f:
            dataset_info = json.load(f)
    except Exception:
        return gr.Button(interactive=False)

    if len(dataset) == 0 or "file_name" not in dataset_info[dataset]:
        return gr.Button(interactive=False)

    data_path = os.path.join(DEFAULT_DATA_DIR, dataset_info[dataset]["file_name"])
...
def get_preview(dataset: list, page_index: int) -> Tuple[int, list, "gr.Column"]:
    with open(os.path.join(DEFAULT_DATA_DIR, DATA_CONFIG), "r", encoding="utf-8") as f:
        dataset_info = json.load(f)
...
def create_preview_box(dataset: "gr.Dropdown") -> Dict[str, "Component"]:
    ...
    dataset.change(can_preview, [dataset], [data_preview_btn], queue=False).then(
        lambda: 0, outputs=[page_index], queue=False
    )
    data_preview_btn.click(
        get_preview, [dataset, page_index], [preview_count, preview_samples, preview_box], queue=False
    )
