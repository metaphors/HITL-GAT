...
# - Commit ID: 4df712e0a5aebc03daa9b1ef353da4b7ea0a1b23
# - File: /OpenAttack/attack_eval/attack_eval.py
...
from OpenAttack.utils import get_language, language_by_name
...
class AttackEval:
    ...
    def eval(self, dataset: Iterable[Dict[str, Any]], total_len : Optional[int] = None, visualize : bool = False, progress_bar : bool = False, num_workers : int = 0, chunk_size : Optional[int] = None, log_dir : str = ""):
        """
        ...
        for i, res in enumerate(result_iterator):
            ...
            if TAG_Classification in self.victim.TAGS:
                ...
                if visualize:
                    if progress_bar:
                        visualizer(i + 1, x_orig, y_orig, x_adv, y_adv, info, tqdm_writer, self.tokenizer, log_dir)
                    else:
                        visualizer(i + 1, x_orig, y_orig, x_adv, y_adv, info, sys.stdout.write, self.tokenizer, log_dir)
